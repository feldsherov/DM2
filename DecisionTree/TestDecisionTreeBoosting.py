...
class DecisionTreeTest(unittest.TestCase):
    ...
    def test_on_housing_dataset(self):
        ...
        for train, test in kf:
            ...
            ensemble = GradientTreeBoosting(count_steps=50, max_tree_depth=10, min_branch_size=0.15, b_coef=0.9)
            ensemble.fit(train_x, train_y)

            ensemble_with_sktree = GradientTreeBoostingViaSklearnTree(count_steps=100, max_tree_depth=10,
                                                                      min_branch_size=0.15, b_coef=0.90)
